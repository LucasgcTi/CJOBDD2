CREATE DATABASE Lista2CJ3027236;
GO
...
SELECT COUNT(*) AS Clientes_Saldo_Alto -- Lucas Gomes
FROM CUSTOMER 
WHERE CUS_BALANCE > 500.00;


SELECT -- Lucas Gomes
    C.CUS_CODE, C.CUS_LNAME, I.INV_NUMBER, I.INV_DATE, P.P_DESCRIPT, L.LINE_UNITS, L.LINE_PRICE
FROM CUSTOMER C
...
SELECT -- Lucas Gomes
    ...
    SUM(InvoiceTotal) AS "Total Sales",
    MIN(InvoiceTotal) AS "Minimum Sale",
    MAX(InvoiceTotal) AS "Largest Sale",
    AVG(InvoiceTotal) AS "Average Sale"
FROM (
    SELECT 
        I.INV_NUMBER,
        SUM(L.LINE_UNITS * L.LINE_PRICE) AS InvoiceTotal
    FROM INVOICE I
    JOIN LINE L ON I.INV_NUMBER = L.INV_NUMBER
    GROUP BY I.INV_NUMBER
) T;

SELECT DISTINCT -- Lucas Gomes
    C.CUS_CODE,
    C.CUS_BALANCE
FROM CUSTOMER C
JOIN INVOICE I ON C.CUS_CODE = I.CUS_CODE
